...
export default function ProfilePage() {
  ...
  return (
    <div dir="rtl" style={{ minHeight: "100vh", background: COLORS.bg, color: COLORS.text, fontFamily: "'Tajawal','Cairo',sans-serif", paddingBottom: 100 }}>
      ...
      {/* HERO */}
      <div style={{ background: "linear-gradient(135deg,#0d1424,#111827)", borderBottom: `1px solid ${COLORS.border}`, textAlign: "center", overflow: "hidden" }}>
        {client?.image_url ? (
          <img src={client.image_url} alt={client.business_name} style={{ width: "100%", height: 220, objectFit: "cover" }} />
        ) : (
          <div style={{ padding: "40px 24px 0" }}>
            <div style={{ width: 80, height: 80, borderRadius: 22, background: "linear-gradient(135deg,#00d4aa,#0070f3)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 40, margin: "0 auto", boxShadow: "0 8px 32px #00d4aa44" }}>{sectorIcon}</div>
          </div>
        )}
        <div style={{ padding: "20px 24px 28px" }}>
          <h1 style={{ fontSize: 28, fontWeight: 900, color: COLORS.white, marginBottom: 10 }}>{client.business_name}</h1>
